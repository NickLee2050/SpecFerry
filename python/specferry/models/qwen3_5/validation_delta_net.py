...
def captured_inputs(trace: Path, steps: int, config=BASELINE):
    ...
    if any(x.shape != (1, 1, config.hidden) or x.dtype != np.float16 for x in values):
...
def prepare(model: Path, trace: Path, destination: Path, steps: int):
    ...
    recurrent = torch.randn(config.recurrent_shape, generator=generator) * 0.01
    convolution = (torch.randn(config.convolution_shape, generator=generator) * 0.02).half()
