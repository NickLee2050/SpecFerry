...
def captured_inputs(trace: Path, steps: int, config=BASELINE):
    if not 2 <= steps <= config.capacity:
    ...
    if not values or any(
        x.shape != (1, 1, config.hidden) or x.dtype != np.float16 or not np.isfinite(x).all()
        for x in values
    ):
    ...
    def attention(module, query, key, value, *args, **kwargs):
        ...
        captured.update(
            ...
            probabilities=torch.nn.functional.pad(
                probabilities, (0, cache.config.capacity - cache.length)
            ),
        )
...
def prepare(model: Path, trace: Path, destination: Path, steps: int):
    ...
    inputs, input_names = captured_inputs(trace, steps, config)
    ...
    observations += reference_sequence(
        mixer, rotary, ReferenceCache(config), inputs[:8], destination, "fresh"
    )
    metadata = {
        "steps": steps,
...
def evaluate_storage(actual: Path, evidence: dict):
    ...
    passed = (
        ...
        and execution.get("cache_payload_bytes") == config.kv_slot_bytes * config.capacity
...
def evaluate(fixture: Path, actual: Path, metadata: dict, evidence: dict):
    ...
    lifecycle = (
        ...
        and execution.get("cache_payload_bytes") == config.kv_slot_bytes * config.capacity
        and execution.get("slot_write_payload_bytes") == config.kv_slot_bytes
