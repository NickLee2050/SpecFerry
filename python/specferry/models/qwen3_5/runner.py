...
from .checkpoint import REVISION, inventory, sha256
from .model import encode, load_text_model, load_tokenizer, source_record
...
def generate(model, tokenizer, prompt_ids, sequential, count, directory):
    ...
    result = {
        ...
        "decode_seconds": times,
        "cache_shapes": shapes,
        "consumed_length": prompt_ids.shape[1] + len(outputs) - 1,
...
def teacher_forcing(model, input_ids, forced_ids, directory):
    ...
    write(
        directory / "teacher-forcing.json",
        {
...
def run(root, output, threads=8, max_new_tokens=32):
    ...
    cases = [
        ("chinese", "中国的首都是哪里？请只回答城市名。"),
        ("english", "What is 2 + 3? Answer briefly."),
    ]
    ...
    policy = {
        ...
        "shared_source_sha256": {
            ...
            for path in (
                Path(__file__).parents[2] / "data/checkpoint.py",
                Path(__file__).parents[2] / "reference/tensors.py",
            )
        },
        ...
        with precision(mode):
            ...
            for case_name, prompt in cases:
                ...
                if ids.shape[1] > 256 or ids.shape[1] + max_new_tokens > 512:
                    raise ValueError(
                        "CPU reference limits exceeded: at most 256 prompt tokens and 512 total tokens"
                    )
                write(
                    case_dir / "input.json",
                    {
                ...
                generated, logits = generate(
                    model, tokenizer, ids, mode == "deployment-fp16", max_new_tokens, case_dir
                )
                ...
                generated["official_prefix_agrees"] = (
                    generated["output_ids"][: len(generated["official_generate_first_ids"])]
                    == generated["official_generate_first_ids"]
                )
                    ...
                    with np.load(prior, allow_pickle=False) as original:
                        ...
                        for key, actual in forced.items():
                            ...
                            if key.startswith("logits."):
                                cross[key]["top1_agrees"] = (
                                    actual.argmax().item() == baseline.argmax().item()
                                )
    ...
    tolerances = {
        **TOLERANCES,
        "holdout_validation_passed": not failures,
