...
class Capture:
    ...
    def attach(self, model):
        ...
        for name in ('causal_conv1d_fn', 'causal_conv1d_update', 'torch_chunk_gated_delta_rule',
                     'torch_recurrent_gated_delta_rule', 'l2norm', 'apply_rotary_pos_emb'):
...
def compare(actual, expected, atol, rtol):
    ...
    return {'pass': not bad.any().item(), 'shape': list(a.shape), 'dtype': str(actual.dtype),
            'max_abs_error': difference.max().item(), 'rmse': difference.square().mean().sqrt().item(),
            'mismatches': bad.sum().item(), 'elements': a.numel(), 'atol': atol, 'rtol': rtol}
