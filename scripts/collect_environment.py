...
def collect(include, libraries):
    ...
    for device in sorted(Path('/sys/bus/pci/devices').glob('*')):
        if read(device / 'vendor') == '0x0709':
            pci.append({'address': device.name, 'vendor': read(device / 'vendor'),
                        'device': read(device / 'device'), 'revision': read(device / 'revision'),
    ...
    if device.exists():
        s = device.stat()
        node.update(character_device=stat.S_ISCHR(s.st_mode), major=os.major(s.st_rdev),
                    minor=os.minor(s.st_rdev), readable=os.access(device, os.R_OK),
                    writable=os.access(device, os.W_OK))
    library_info = []
    for name in ('libovxlib.so', 'libOpenVX.so', 'libGAL.so', 'libArchModelSw.so', 'libNNArchPerf.so'):
        path = libraries / name
    ...
    return {
        'recorded_at': datetime.now(timezone.utc).isoformat(), 'hostname': platform.node(),
        'platform': platform.platform(), 'machine': platform.machine(),
        'python': sys.version, 'python_executable': sys.executable, 'cpu_count': os.cpu_count(),
        'cpu': command(['lscpu']), 'memory': read('/proc/meminfo'),
        'disk_free_bytes': free, 'disk_reserve_8gib_pass': free >= 8 * 1024**3,
        'sdk_include_dir': str(include), 'sdk_library_dir': str(libraries),
        ...
        'tools': {name: command(args) for name, args in {
            'c_compiler': ['gcc', '--version'], 'cxx_compiler': ['g++', '--version'],
            'compiler_target': ['g++', '-dumpmachine'], 'cmake': ['cmake', '--version'],
            'strace': ['strace', '--version']}.items()},
        'pci_devices': pci, 'device_node': node,
        'galcore_module_loaded': Path('/sys/module/galcore').exists(),
        'driver_parameters': {name: read(Path('/sys/module/galcore/parameters') / name)
                              for name in ('contiguousSize', 'externalSize', 'physSize', 'enableNN', 'type')},
        'runtime_options': {name: os.environ.get(name) for name in
                            ('VIV_VX_ENABLE_PRINT_TARGET', 'VIV_VX_PROFILE', 'VIV_VX_ENABLE_SHADER')},
...
def main():
    ...
    print(json.dumps({'environment': str(args.output / 'environment.json'),
                      'disk_reserve_pass': result['disk_reserve_8gib_pass'], 'pci_devices': len(result['pci_devices'])}))
