...
def main():
    ...
    parser.add_argument(
        "--check", action="store_true", help="report formatting differences without modifying files"
    )
    parser.add_argument(
        "--clang-format", default="clang-format", help="clang-format executable name or path"
    )
    args = parser.parse_args()
    formatter = shutil.which(args.clang_format)
    if formatter is None:
        parser.error(
            "clang-format was not found; activate SpecFerry and install dependencies "
