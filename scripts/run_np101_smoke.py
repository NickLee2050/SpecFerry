...
def main():
    ...
    if tracer:
        command = [tracer, '-f', '-tt', '-T', '-yy', '-e', 'trace=openat,close,ioctl,mmap',
                   '-o', str(output / 'smoke.strace'), *command]
    evidence = {'started_at': datetime.now(timezone.utc).isoformat(), 'command': command,
                'binary_sha256': hashlib.sha256(binary.read_bytes()).hexdigest(),
                'binary_elf': subprocess.run(['readelf', '-h', str(binary)], capture_output=True, text=True).stdout,
                'linked_libraries': subprocess.run(['ldd', str(binary)], env=env, capture_output=True, text=True).stdout,
                'sdk_library_dir': str(args.sdk_lib.resolve()), 'print_target': True, 'timeout': False}
    ...
    evidence['note'] = 'Generic device ioctl success proves driver interaction, not individual node execution or absence of CPU/software paths.'
    (output / 'smoke-evidence.json').write_text(json.dumps(evidence, indent=2) + '\n')
    print(json.dumps({'returncode': code, 'driver_ioctls': len(successes),
                      'hardware_acceptance': evidence['hardware_acceptance'], 'output': str(output)}))
