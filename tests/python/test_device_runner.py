...
class ExecutionArtifactTests(unittest.TestCase):
    ...
    def test_snapshot_preserves_executable_bytes_and_mode_across_rebuilds(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / "custom-probe"
            source.write_bytes(b"original executable")
            source.chmod(0o755)
            snapshot = device.snapshot_binary(source, root / "run-probe")
            source.write_bytes(b"rebuilt executable")
            self.assertEqual(snapshot.read_bytes(), b"original executable")
            self.assertEqual(snapshot.stat().st_mode & 0o777, 0o755)
            self.assertNotEqual(device.fingerprint(snapshot), device.fingerprint(source))

    def test_manifest_records_snapshot_and_working_tree_with_relative_paths(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            sources = {
                "native/np101/tensor.cpp": b"native source",
                "native/np101/tensor.hpp": b"native header",
                "python/specferry/validation/device.py": b"python source",
                "scripts/check.py": b"script source",
                "tests/python/test_device.py": b"test source",
            }
            for name, contents in sources.items():
                path = root / name
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(contents)
            (root / "tests/fixture.bin").write_bytes(b"generated bytes")
            (root / "tests/report.md").write_text("report")
            build = root / "build"
            build.mkdir()
            (build / "generated.cpp").write_text("not project source")
            binary = build / "probe"
            binary.write_bytes(b"original executable")
            snapshot = device.snapshot_binary(binary, root / "run-probe")
            binary.write_bytes(b"rebuilt executable")

            report = root / "sources.json"
            device.record_sources(root, snapshot, report)
            actual = json.loads(report.read_text())
            self.assertEqual(
                actual["binary_sha256"], hashlib.sha256(b"original executable").hexdigest()
            )
            self.assertEqual(
                actual["sources"],
                {name: hashlib.sha256(contents).hexdigest() for name, contents in sources.items()},
            )
